import pygame
import random
import Tetris.tetrisData as tData
import user
...
def Run(CURRENT_USER: user.User):
	...
	img1 = pygame.image.load('Tetris/Assets/1.png')
	img2 = pygame.image.load('Tetris/Assets/2.png')
	img3 = pygame.image.load('Tetris/Assets/3.png')
	img4 = pygame.image.load('Tetris/Assets/4.png')
	...
	font = pygame.font.Font('Tetris/Fonts/Alternity-8w7J.ttf', 50)
	...
	while running:
		...
		if tetris.gameover:
			rect = pygame.Rect((50, 100, WIDTH-100, HEIGHT-280))
			pygame.draw.rect(win, BLACK, rect)
			pygame.draw.rect(win, RED, rect, 2)
			if not tetris.resultSaved:
				isHighScore = tData.IsNewHighScore(tetris.score)
				if not isHighScore:
					data = tData.GetHighScores(CURRENT_USER.ID)				
				tData.AddScore(CURRENT_USER.ID,tetris.score)
				tetris.resultSaved = True
			...
			if isHighScore:
				highscore_img = font2.render("NEW", True, WHITE)
				highscore_img2 = font2.render("GLOBAL HIGHSCORE", True, WHITE)
			else:				
				highscore_img = font2.render(f"Global Highscore: {data[0]}", True, WHITE)
				highscore_img2 = font2.render(f"Personal Highscore: {data[1]}", True, WHITE)

			message_x = rect.centerx - msg2.get_width()/2
			...
			win.blit(highscore_img, (rect.centerx - highscore_img.get_width()/2, rect.y + 150))
			win.blit(highscore_img2, (rect.centerx - highscore_img2.get_width()/2, rect.y + 180))
