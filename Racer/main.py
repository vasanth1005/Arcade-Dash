import pygame
import random
from Racer.objects import *
import user
import Racer.racerData as rcData

def Run(CURRENT_USER: user.User):
	pygame.init()
	SCREEN = WIDTH, HEIGHT = 300, 512
	...
	font = pygame.font.SysFont('cursive', 28)

	select_car = font.render('Select Car', True, WHITE)

	# IMAGES **********************************************************************

	bg = pygame.image.load('Racer/Assets/bg.png')

	home_img = pygame.image.load('Racer/Assets/home.png')
	play_img = pygame.image.load('Racer/Assets/buttons/play.png')
	end_img = pygame.image.load('Racer/Assets/end.jpg')
	end_img = pygame.transform.scale(end_img, (WIDTH, HEIGHT))
	game_over_img = pygame.image.load('Racer/Assets/game_over.png')
	game_over_img = pygame.transform.scale(game_over_img, (220, 220))
	coin_img = pygame.image.load('Racer/Assets/coins/1.png')
	dodge_img = pygame.image.load('Racer/Assets/car_dodge.png')

	left_arrow = pygame.image.load('Racer/Assets/buttons/arrow.png')
	right_arrow = pygame.transform.flip(left_arrow, True, False)

	home_btn_img = pygame.image.load('Racer/Assets/buttons/home.png')
	replay_img = pygame.image.load('Racer/Assets/buttons/replay.png')
	sound_off_img = pygame.image.load("Racer/Assets/buttons/soundOff.png")
	sound_on_img = pygame.image.load("Racer/Assets/buttons/soundOn.png")

	cars = []
	car_type = 0
	for i in range(1, 9):
		img = pygame.image.load(f'Racer/Assets/cars/{i}.png')
	...
	nitro_counter = 0
	for i in range(6):
		img = pygame.image.load(f'Racer/Assets/nitro/{i}.gif')
	...
	click_fx = pygame.mixer.Sound('Racer/Sounds/click.mp3')
	fuel_fx = pygame.mixer.Sound('Racer/Sounds/fuel.wav')
	start_fx = pygame.mixer.Sound('Racer/Sounds/start.mp3')
	restart_fx = pygame.mixer.Sound('Racer/Sounds/restart.mp3')
	coin_fx = pygame.mixer.Sound('Racer/Sounds/coin.mp3')

	pygame.mixer.music.load('Racer/Sounds/mixkit-tech-house-vibes-130.mp3')
	...
	while running:
		...
		if over_page:
			if not saved:
				isHighScore = rcData.IsNewHighScore(distance)
				if not isHighScore:
					data = rcData.GetHighScores(CURRENT_USER.ID)				
				rcData.AddScore(CURRENT_USER.ID,distance)
				saved = True
			...
			distance_img = font.render(f'Distance : {distance/1000:.2f} km', True, WHITE)
			if isHighScore:
				highscore_img = font.render("NEW GLOBAL HIGHSCORE", True, WHITE)
				highscore_img2 = font.render("", True, WHITE)
			else:				
				highscore_img = font.render(f"Global Highscore: {data[0]/1000:.2f} km", True, WHITE)
				highscore_img2 = font.render(f"Personal Highscore: {data[1]/1000:.2f} km", True, WHITE)

			win.blit(coin_img, (80, 210))
			win.blit(dodge_img, (50, 250))
			win.blit(num_coin_img, (180, 220))
			win.blit(num_dodge_img, (180, 270))
			win.blit(distance_img, (center(distance_img), (320)))
			win.blit(highscore_img, (10, 350))
			win.blit(highscore_img2, (10, 380))
